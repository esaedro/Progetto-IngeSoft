...
public class InputDati {
	...
	private final static char[] CARATTERI_AMMISSIBILI = {'S', 's', 'N', 'n'};
	...
	public static <T> T selezionaUnoDaLista(String messaggioInserimento, Iterable<T> lista, Function<T, String> toString) {
		System.out.print(ANSI_RESET);
		String selezionato = ANSI_GREEN + "*" + ANSI_RESET;
		char nonSelezionato = ' ';
		...
		while (!finito) {
			System.out.print(PULISCI_TERMINALE); // Probabilmente non funzionerà su Windows, in caso è possibile sostituirla con una sfilza di println
 ...
 */
		...
		do {
			...
			try {
                ...
                if (giornoInserito != Integer.parseInt(dataInserita.split(separatore)[0])) {
                    throw new ParseException(ANSI_RED + "Data non valida" + ANSI_RESET, 0);
				} else {
					finito = true;
				}
	 ...
	 * @return la data letta come oggetto Calendar
	 */
	public static Calendar leggiDataMinMax(String messaggioInserimento, String separatore, Calendar minimo, Calendar massimo) {
		System.out.print(ANSI_RESET);
		...
		String messaggioInserimento2 = messaggioInserimento + " (minimo: " + formattatore.format(minimo.getTime()) + ", massimo: " + formattatore.format(massimo.getTime()) + ")";
		do {
			...
			if (data.after(minimoFittizio) && data.before(massimo))
				finito = true;
	...
	public static Calendar leggiOra(String messaggioInserimento, String separatore) {
		...
		do {
			...
			try {
				...
				if (ora.get(Calendar.HOUR_OF_DAY) != Integer.parseInt(oraInserita.split(separatore)[0])) {
					throw new ParseException(ANSI_RED + "Ora non valida" + ANSI_RESET, 0);
				} else if (ora.get(Calendar.MINUTE) != Integer.parseInt(oraInserita.split(separatore)[1])) {
					throw new ParseException(ANSI_RED + "Ora non valida" + ANSI_RESET, 0);
				} else {
					finito = true;
				}
	 ...
	 * @return l'ora letta come oggetto Calendar
	 */
	public static Calendar leggiOraMinMax(String messaggioInserimento, String separatore, Calendar minimo, Calendar massimo) {
		...
		do {
			...
			if (ora.after(minimoFittizio) && ora.before(massimo))
				finito = true;

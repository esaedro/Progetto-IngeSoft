...
public class InputDati {
	...
	private final static char[] CARATTERI_AMMISSIBILI = {'S', 's'};
	...
	public static char leggiUpperChar(String messaggioInserimento, char[] ammissibili) {
		...
		do {
			...
			if (String.valueOf(ammissibili).indexOf(valoreLetto) != -1)
				finito = true;
			else
				System.out.println(MESSAGGIO_AMMISSIBILI + ammissibili.toString());
		} while (!finito);
		return valoreLetto;
	}

	public static char leggiUpperChar(String messaggioInserimento, char[] ammissibili, String messaggioErrore) {
		boolean finito = false;
		char valoreLetto = '\0';
		do {
			valoreLetto = leggiChar(messaggioInserimento);
	...
	public static int leggiInteroMin(String messaggioInserimento, int minimo, String messaggioErrore) {
		boolean finito = false;
		int valoreLetto = 0;
		do {
			valoreLetto = leggiIntero(messaggioInserimento);
	...
	public static int leggiInteroMinMax(String messaggioInserimento, int minimo, int massimo, String messaggioErrore) {
		boolean finito = false;
		int valoreLetto = 0;
		do {
			valoreLetto = leggiIntero(messaggioInserimento);
	...
	public static short leggiShortMin(String messaggioInserimento, int minimo, String messaggioErrore) {
		boolean finito = false;
		short valoreLetto = 0;
		do {
			valoreLetto = leggiShort(messaggioInserimento);
	...
	public static short leggiShortMinMax(String messaggioInserimento, int minimo, int massimo, String messaggioErrore) {
		boolean finito = false;
		short valoreLetto = 0;
		do {
			valoreLetto = leggiShort(messaggioInserimento);
	...
	public static long leggiLongMin(String messaggioInserimento, int minimo, String messaggioErrore) {
		boolean finito = false;
		long valoreLetto = 0;
		do {
			valoreLetto = leggiLong(messaggioInserimento);
	...
	public static long leggiLongMinMax(String messaggioInserimento, int minimo, int massimo, String messaggioErrore) {
		boolean finito = false;
		long valoreLetto = 0;
		do {
			valoreLetto = leggiLong(messaggioInserimento);
	...
	public static double leggiDoubleConMinimo(String messaggioInserimento, double minimo, String messaggioErrore) {
		boolean finito = false;
		double valoreLetto = 0;
		do {
			valoreLetto = leggiDouble(messaggioInserimento);
	...
	public static boolean conferma(String messaggioInserimento) {
		String nuovoMessaggio = messaggioInserimento + "\n[" + CARATTERI_AMMISSIBILI[0] + "/" + CARATTERI_AMMISSIBILI[2]
				+ "]: ";
		char valoreLetto = leggiChar(nuovoMessaggio);

		return (valoreLetto == CARATTERI_AMMISSIBILI[0] || valoreLetto == CARATTERI_AMMISSIBILI[1]) ? true : false;
	}
	 ...
	 *         altrimenti
	 */
	public static <T> T selezionaUnoDaLista(String messaggioInserimento, Iterable<T> lista, Function<T, String> toString) {
		char selezionato = '*';
		char nonSelezionato = ' ';
		...
		while (!finito) {
			System.out.print("\033\143"); // Probabilmente non funzionerà su Windows, in caso è possibile sostituirla con una sfilza di println
			...
			for (int i = 0; i < listaFittizia.size(); i++) {
				...
				riga = String.format("%d) [%s] %s", i + 1,
						selezionati.get(elemento) ? selezionato : nonSelezionato, toString.apply(elemento));
			...
			try {
				if (Integer.parseInt(carattereInserito) > 0 && Integer.parseInt(carattereInserito) <= listaFittizia.size()) {
					elementoSelezionato = listaFittizia.get(Integer.parseInt(carattereInserito) - 1);
		...
		return selezionati.entrySet().stream().filter(entry -> entry.getValue()).findFirst().map(Map.Entry::getKey).orElse(null);
	}
	 ...
	 */
	public static <T> Set<T> selezionaPiuDaLista(String messaggioInserimento, Iterable<T> lista,
			Function<T, String> toString, int minimo, int massimo) {
		char selezionato = '*';
		char nonSelezionato = ' ';
		...
		while (!finito) {
			...
			if (!erroreInterno.isEmpty())
				System.out.println(erroreInterno);
			System.out.println(messaggioInserimento + " (minimo: " + minimo + ", massimo: " + massimo + ")");
			...
			carattereInserito = leggiStringa("('q' per terminare) > ");
			try {
				if (Integer.parseInt(carattereInserito) > 0
		...
		return selezionati.entrySet().stream().filter(entry -> entry.getValue()).map(Map.Entry::getKey)
 ...
 */
	...
	public static <E extends Enum<E>> E selezionaUnEnum(String messaggioInserimento, E[] lista) {
		...
		while (!finito) {
			...
			for (int i = 0; i < lista.length; i++) {
				E elemento = lista[i];
				riga = String.format("%d) [%s] %s", i + 1,
						selezionati.get(elemento) ? selezionato : nonSelezionato, elemento.toString());
				System.out.println(riga);
			}
		...
		return selezionati.entrySet().stream().filter(entry -> entry.getValue()).findFirst().map(Map.Entry::getKey)
				.orElse(null);
	}
	 ...
	 */
	public static <E extends Enum<E>> Set<E> selezionaPiuEnum(String messaggioInserimento, E[] lista, int minimo,
			int massimo) {
		char selezionato = '*';
		char nonSelezionato = ' ';
 ...
 */
	public static Calendar leggiData(String messaggioInserimento) {
		boolean finito = false;
		...
		do {
			...
			try {
				data.setTime(new SimpleDateFormat("dd/MM/yyyy").parse(dataInserita));
				finito = true;
	 ...
	 * @return la data letta come oggetto Calendar
	 */
	public static Calendar leggiDataMinMax(String messaggioInserimento, Calendar minimo, Calendar massimo) {
		boolean finito = false;
		Calendar data = Calendar.getInstance();
		do {
			data = leggiData(messaggioInserimento + " (minimo: " + new SimpleDateFormat("dd/MM/yyyy").format(minimo.getTime()) + ", massimo: " + new SimpleDateFormat("dd/MM/yyyy").format(massimo.getTime()) + ") ");
			if (data.after(minimo) && data.before(massimo))
				finito = true;
			else if (data.before(minimo))
				System.out.println(ERRORE_MINIMO + new SimpleDateFormat("dd/MM/yyyy").format(minimo.getTime()));
			else
				System.out.println(ERRORE_MASSIMO + new SimpleDateFormat("dd/MM/yyyy").format(massimo.getTime()));
		} while (!finito);
		return data;
	}

	public static Calendar leggiOra(String messaggioInserimento) {
		boolean finito = false;
		...
		do {
			...
			try {
				ora.setTime(new SimpleDateFormat("HH:mm").parse(oraInserita));
				finito = true;
			} catch (ParseException e) {
				System.out.println(ERRORE_FORMATO);
			}
	...
	public static Calendar leggiOraMinMax(String messaggioInserimento, Calendar minimo, Calendar massimo) {
		boolean finito = false;
		Calendar ora = Calendar.getInstance();
		do {
			ora = leggiOra(messaggioInserimento + " (minimo: " + new SimpleDateFormat("HH:mm").format(minimo.getTime()) + ", massimo: " + new SimpleDateFormat("HH:mm").format(massimo.getTime()) + ") ");
			if (ora.after(minimo) && ora.before(massimo))
				finito = true;
			else if (ora.before(minimo))
				System.out.println(ERRORE_MINIMO + new SimpleDateFormat("HH:mm").format(minimo.getTime()));
			else
				System.out.println(ERRORE_MASSIMO + new SimpleDateFormat("HH:mm").format(massimo.getTime()));
		} while (!finito);
		return ora;
	}
